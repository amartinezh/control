...
<body class="">
							...
							<div role="content">
								...
								<div class="widget-body">
									<form:form id="frm" method="post" class="bv-form"
										ModelAttribute="tipotransaccion" commandName="tipotransaccion">
										<button type="submit" class="bv-hidden-submit"
											style="display: none; width: 0px; height: 0px;"></button>
										<fieldset>
											<legend> Tipo Transacción </legend>
											<div class="form-group">
												<div class="row">
													<div class="col-md-12 has-feedback">
														<form:input type="hidden" path="tipo_transaccion_id" value="0" />
					...
					<div class="widget-body no-padding">
						...
						<div id="datatable_fixed_column_wrapper"
							...
							<table id="datatable_fixed_column" name="datatable_fixed_column"
								...
								<tbody>
									<c:forEach items="${listTipoTransaccion}" var="obj"
										varStatus="loopCounter">
										<tr role="row" class="odd">
											<td class="sorting_1"><span class="responsiveExpander"></span>
												<a class="btn btn-success btn-circle btn-sx"
												onclick="con('<c:out value="${obj.tipo_transaccion_id}"></c:out>','<c:out value="${obj.descripcion}"></c:out>',$(this))"><i
													class="fa fa-edit"></i></a> <a
												class="btn btn-danger btn-circle"
												onclick="borrar(<c:out value="${obj.tipo_transaccion_id}"></c:out>, $(this))"><i
	...
	<script type="text/javascript">
		...
		function actualizar() {
			$( "#frm" ).submit();
			var acti_id = document.getElementById('tipo_transaccion_id').value;
			var des = document.getElementById('descripcion').value;
			$.ajax({
				type : "POST",
				url : "tipotransaccion/agregar",
				data : {
					tipo_transaccion_id: acti_id,
					descripcion : des
				},
				success : function(data) {			
					 document.getElementById('descripcion').value = "";
					 document.getElementById('tipo_transaccion_id').value = "0";
					 var res = data.split(":::");
		...
		function del(dato, thi) {
			$.ajax({
				type : "POST",
				url : "tipotransaccion/borrar",
				data : {
					tipo_transaccion_id : dato
				},
				success : function(data) {
		...
		function cancelar() {
			var obj_id = document.getElementById('tipo_transaccion_id').value;
			var des = document.getElementById('estado').value;
			$('#elboton').text('Nuevo');
			$.ajax({
				type : "POST",
				url : "tipotransaccion/cancelar",
				data : {
					tipo_transaccion_id: obj_id,
					descripcion : des
				},
				success : function(data) {					
					document.getElementById('descripcion').value = "";
		...
		function con(acti_id, descripcion, thi) {
			document.getElementById('tipo_transaccion_id').value=acti_id;
